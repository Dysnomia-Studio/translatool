...
export default function TranslationListView({ file, list, lang, direction, setLanguage, selectedFolder }) {
	...
	function updateEntry(event, id) {
		...
		saveTranslation(selectedFolder, file, lang, list);
	}
	...
	return (
		<div className={`languageSelector-body languageSelector-body-${direction}`}>
			<LanguageSelector current={lang} setLanguage={setLanguage} selectedFolder={selectedFolder} />
			{values}
		</div>
	);
}
